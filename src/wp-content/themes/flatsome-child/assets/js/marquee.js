function marqueeInit(config) {
  ...
  marqueeInit.run(config.uniqueid, config.type);
}
...
(function () {
  ...
  marqueeInit.run = function (id, type) {
    if (type == "class") {
      if (
        ie &&
        !marqueeInit.OK &&
        iever < 8 &&
        intable(document.getElementsByClassName(id)[0])
      ) {
      ...
      else new Marq(c++, document.getElementsByClassName(id)[0]);
    }else{
      if (
        ie &&
        !marqueeInit.OK &&
